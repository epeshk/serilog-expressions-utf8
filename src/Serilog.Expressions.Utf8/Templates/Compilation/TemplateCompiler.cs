        ...
        Utf8TemplateTheme theme)
    {
        return template switch
        {
            ...
            FormattedExpression { Expression: AmbientNameExpression { IsBuiltIn: true, PropertyName: BuiltInProperty.Level} } level =>
                new CompiledLevelToken(level.Format, level.Alignment, theme),
            FormattedExpression
            {
            ...
            //     Alignment: not null
            // } expression => new CompiledAlignedTimestampToken(expression.Format, expression.Alignment.GetValueOrDefault(), formatProvider, theme),
            FormattedExpression
            {
                Expression: AmbientNameExpression { IsBuiltIn: true, PropertyName: BuiltInProperty.Timestamp }
            } expression => new CompiledTimestampToken(expression.Format, expression.Alignment, formatProvider, theme),
            FormattedExpression expression => 
                new CompiledFormattedExpression(ExpressionCompiler.Compile(expression.Expression, formatProvider, nameResolver), expression.Format, expression.Alignment, formatProvider, theme),
